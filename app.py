...
"""
COMPLETE TAB 2 REPLACEMENT CODE
Paste this to replace your entire Tab 2 section
"""

with tab2:
    st.markdown("## 📊 Newsletter Performance Dashboard")
    ...
    def render_metric_row(label, daily, daily_pct, weekly, weekly_pct, monthly, monthly_pct):
        """Render a metric row"""
        cols = st.columns([2, 1.5, 1, 1.5, 1, 1.5, 1])
    ...
    with st.expander("📅 Date Ranges", expanded=False):
        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(f"**Yesterday:** {yesterday.strftime('%b %d')}")
            st.markdown(f"**Previous Day:** {day_before_yesterday.strftime('%b %d')}")
        with col2:
            st.markdown(f"**This Week:** {week_start.strftime('%b %d')} - {week_end.strftime('%b %d')}")
            st.markdown(f"**Last Week:** {prev_week_start.strftime('%b %d')} - {prev_week_end.strftime('%b %d')}")
        with col3:
            st.markdown(f"**This Month:** {month_start.strftime('%b %Y')}")
            st.markdown(f"**Last Month:** {prev_month_start.strftime('%b %Y')}")
    
    st.markdown("---")
    
    # === TINYEMAIL ACCOUNT WIDE ===
    st.markdown("## 📧 TinyEmail Account Wide")
    
    tiny_yesterday = get_metrics_for_period(yesterday, yesterday, platform='tinyemail')
    tiny_day_before = get_metrics_for_period(day_before_yesterday, day_before_yesterday, platform='tinyemail')
    tiny_this_week = get_metrics_for_period(week_start, week_end, platform='tinyemail')
    tiny_last_week = get_metrics_for_period(prev_week_start, prev_week_end, platform='tinyemail')
    tiny_this_month = get_metrics_for_period(month_start, month_end, platform='tinyemail')
    tiny_last_month = get_metrics_for_period(prev_month_start, prev_month_end, platform='tinyemail')
    ...
    if tiny_yesterday and tiny_this_week and tiny_this_month:
        metrics = ['sends', 'delivered', 'opens', 'unique_opens', 'clicks', 'unique_clicks', 'spam_reports']
        labels = ['Sends', 'Delivered', 'Opens', 'Unique Opens', 'Clicks', 'Unique Clicks', 'Spam Reports']
        
        for label, metric in zip(labels, metrics):
            daily = tiny_yesterday.get(metric, 0)
            daily_prev = tiny_day_before.get(metric, 0) if tiny_day_before else 0
            daily_pct = calculate_pct_change(daily, daily_prev)
            
            weekly = tiny_this_week.get(metric, 0)
            weekly_prev = tiny_last_week.get(metric, 0) if tiny_last_week else 0
            weekly_pct = calculate_pct_change(weekly, weekly_prev)
            
            monthly = tiny_this_month.get(metric, 0)
            monthly_prev = tiny_last_month.get(metric, 0) if tiny_last_month else 0
            monthly_pct = calculate_pct_change(monthly, monthly_prev)
            
            render_metric_row(label, daily, daily_pct, weekly, weekly_pct, monthly, monthly_pct)
    else:
        st.info("No TinyEmail data available")
    
    st.markdown("---")
    
    # === BEEHIIV ACCOUNT WIDE ===
    st.markdown("## 🐝 Beehiiv Account Wide")
    
    bee_yesterday = get_metrics_for_period(yesterday, yesterday, platform='beehiiv')
    bee_day_before = get_metrics_for_period(day_before_yesterday, day_before_yesterday, platform='beehiiv')
    bee_this_week = get_metrics_for_period(week_start, week_end, platform='beehiiv')
    bee_last_week = get_metrics_for_period(prev_week_start, prev_week_end, platform='beehiiv')
    bee_this_month = get_metrics_for_period(month_start, month_end, platform='beehiiv')
    bee_last_month = get_metrics_for_period(prev_month_start, prev_month_end, platform='beehiiv')
    ...
    if bee_yesterday and bee_this_week and bee_this_month:
        for label, metric in zip(labels, metrics):
            daily = bee_yesterday.get(metric, 0)
            daily_prev = bee_day_before.get(metric, 0) if bee_day_before else 0
            daily_pct = calculate_pct_change(daily, daily_prev)
            
            weekly = bee_this_week.get(metric, 0)
            weekly_prev = bee_last_week.get(metric, 0) if bee_last_week else 0
            weekly_pct = calculate_pct_change(weekly, weekly_prev)
            
            monthly = bee_this_month.get(metric, 0)
            monthly_prev = bee_last_month.get(metric, 0) if bee_last_month else 0
            monthly_pct = calculate_pct_change(monthly, monthly_prev)
            
            render_metric_row(label, daily, daily_pct, weekly, weekly_pct, monthly, monthly_pct)
    else:
        st.info("No Beehiiv data available")
    
    st.markdown("---")
    
    # === COMBINED ACCOUNTS ===
    st.markdown("## 🌐 Combined Accounts")
    
    combined_yesterday = get_metrics_for_period(yesterday, yesterday)
    combined_day_before = get_metrics_for_period(day_before_yesterday, day_before_yesterday)
    combined_this_week = get_metrics_for_period(week_start, week_end)
    combined_last_week = get_metrics_for_period(prev_week_start, prev_week_end)
    combined_this_month = get_metrics_for_period(month_start, month_end)
    combined_last_month = get_metrics_for_period(prev_month_start, prev_month_end)
    ...
    if combined_yesterday and combined_this_week and combined_this_month:
        for label, metric in zip(labels, metrics):
            daily = combined_yesterday.get(metric, 0)
            daily_prev = combined_day_before.get(metric, 0) if combined_day_before else 0
            daily_pct = calculate_pct_change(daily, daily_prev)
            
            weekly = combined_this_week.get(metric, 0)
            weekly_prev = combined_last_week.get(metric, 0) if combined_last_week else 0
            weekly_pct = calculate_pct_change(weekly, weekly_prev)
            
            monthly = combined_this_month.get(metric, 0)
            monthly_prev = combined_last_month.get(metric, 0) if combined_last_month else 0
            monthly_pct = calculate_pct_change(monthly, monthly_prev)
    ...
    if combined_yesterday:
        ...
        with col1:
            st.metric("Yesterday Sends", f"{combined_yesterday.get('sends', 0):,}")
        
        with col2:
            sends = combined_yesterday.get('sends', 0)
            opens = combined_yesterday.get('opens', 0)
            open_rate = (opens / sends * 100) if sends > 0 else 0
            st.metric("Open Rate", f"{open_rate:.1f}%")
        
        with col3:
            clicks = combined_yesterday.get('clicks', 0)
            ctr = (clicks / sends * 100) if sends > 0 else 0
            st.metric("CTR", f"{ctr:.2f}%")
        
        with col4:
            st.metric("Active List", f"{combined_yesterday.get('list_size', 0):,}")
